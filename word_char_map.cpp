...
std::map<string,int> readTxt(const char *s,map<string,int>temp);//从.txt文件读词库到map
...
int main()
{
	...
	/*map<string,int>::iterator it=myMapChar.begin();//遍历myCharMap,输出myMapChar
	for(;it!=myMapChar.end();++it)
		cout<<"key:"<<it->first<<"value:"<<it->second<<endl; */

	myMapWord=readTxt(s2,myMapWord);
	/*map<string,int>::iterator it2=myMapWord.begin();//遍历myWordMap,输出myMapWord
	for(;it2!=myMapWord.end();++it2)
